...
OUT_CSV = os.path.join(SCRIPT_DIR, "graphld_ldl_neutral_and_replicates_wc_postmean_plei.csv")
OUT_PDF = os.path.join(SCRIPT_DIR, "graphld_ldl_neutral_and_replicates_wc_postmean_plei_pvals.pdf")
MIN_X = 0.01
P_THRESH = 5e-8
N_E = 10000
...
def fit_plei(sfs_pile, spec):
    ...
    ll_plei = -fit.fun
    lrt_plei = 2 * (ll_plei - ll_neut)
    return {
        "n": int(len(df)),
        "n_eff": float(spec["n_eff"]),
        "v_cut": float(v_cut),
        "ll_neut": float(ll_neut),
        "ll_plei": float(ll_plei),
        "Ip_plei": float(10 ** fit.x[0]),
        "dLL_plei": float(ll_plei - ll_neut),
        "dLL_per_locus_plei": float((ll_plei - ll_neut) / len(df)),
        "stat_plei": float(stable_chi2_log10sf(lrt_plei, 1)),
    }
...
def plot_pvals(fit_df):
    ...
    for condition in ["neutral", "selected"]:
        ...
        causal_condition = causal_condition.sort_values("stat_plei", ascending=False).reset_index(drop=True)
    ...
    ax.set_ylim(-0.1, max(4.0, 0.35 * fit_df["stat_plei"].max()) + fit_df["stat_plei"].max())
...
def main():
    ...
    for rep_index, path in enumerate(neutral_paths, start=1):
        ...
        for dataset in ["causal", "beta_p", "beta_p_wc", "beta_p_post"]:
            ...
            row = fit_plei(sfs_pile, specs[dataset])
    ...
    print(" -", OUT_PDF)
    print(
        fit_df.groupby(["condition", "dataset"])[["n", "dLL_plei", "dLL_per_locus_plei", "stat_plei", "Ip_plei"]]
        .median()
        .reset_index()
