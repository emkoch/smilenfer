...
traits = ["ibd", "scz", "malignant_neoplasms", "arthrosis"]
trait_labels = ["IBD", "SCZ", "Malignant neoplasms", "Arthrosis"]
min_x = 0.01
p_thresh = 5e-08
p_cutoff = 5e-08
...
ML = ML[ML["beta"]=="ash"]
sel_params = [float(ML.loc[ML.trait==trait.upper(),"Ip_plei"]) for trait in traits]
...
for i, trait in enumerate(traits):
    ...
    _, _, cbar, _ = splot.plot_smile_fit(raf=trait_df.raf.to_numpy(), 
                               ...
                               v_cut=v_cut,
                               model="plei",
                               params={"Ne":Ne, "Ip":float(sel_params[i])}, 
                               WF_pile=WF_pile,
...
for i, trait in enumerate(traits):
    ...
    axes[1,i].set_yticklabels(["{:.3f}".format(float(t.get_text())) for t in axes[0,i].get_yticklabels()])
