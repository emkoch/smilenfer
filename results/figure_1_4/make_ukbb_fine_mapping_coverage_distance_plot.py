...
ORIGINAL_DIR = "../data/final/original_traits"
SUSIEX_DIR = "../data/final/UKBB_susiex"
...
def load_trait_data(trait):
    original_path = f"{ORIGINAL_DIR}/processed.{trait}.snps_low_r2.tsv"
    susiex_path = f"{SUSIEX_DIR}/susiex_cs_table_{trait}.csv"
...
def plot_trait_panel(ax, trait, summary_df, negative_n, x_left, x_right):
...
trait_data = {trait: load_trait_data(trait) for trait in TRAITS}
trait_summaries = {
    trait: summarize_trait(trait_data[trait])
    for trait in TRAITS
}

all_distance = np.concatenate([trait_data[trait]["power_distance_log10"].to_numpy() for trait in TRAITS])
...
for ii, trait in enumerate(TRAITS):
    summary_df, negative_n = trait_summaries[trait]
    plot_trait_panel(axes[ii], trait, summary_df, negative_n, x_left, x_right)
    if ii % 4 == 0:
        axes[ii].set_ylabel("Mapped rate")

fig.tight_layout()
fig.savefig("ukbb_vs_susiex_power_distance_mapping.pdf", bbox_inches="tight")
